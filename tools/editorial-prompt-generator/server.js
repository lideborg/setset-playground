...
// Serve the main navigation page as the root (before static middleware)
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, '../../../index.html'));
});

// Serve remix.html
app.get('/remix.html', (req, res) => {
  res.sendFile(path.join(__dirname, 'public/remix.html'));
});

// Serve from the PromptTest root directory to access all projects
...
// ===== NANO BANANA PRO REMIX API =====
app.post('/api/remix', async (req, res) => {
  try {
    const { prompt, image_urls, num_images = 1, aspect_ratio, resolution } = req.body;

    if (!prompt || !image_urls || image_urls.length === 0) {
      ...
      image_urls.map(async (url) => {
        if (url.startsWith('data:')) {
          try {
            ...
            console.log(`Uploaded base64 image to fal storage`);
            return uploadedUrl;
          } catch (uploadError) {
        ...
        return url; // Return as-is if not base64
      })
    );

    const startTime = Date.now();

    // Retry logic for handling temporary quota errors
    const maxRetries = 3;
    let lastError = null;
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // The fal client returns the result in result.data
        const data = result.data || result;
        const images = data.images || [];

        console.log('\n✅ === NANO BANANA PRO REMIX COMPLETE ===');
        console.log(`⏱️  Duration: ${duration}s`);
        console.log(`📊 Generated: ${images.length} image${images.length !== 1 ? 's' : ''}`);

        // Log generated image details with Setset naming convention
        if (images.length > 0) {
          const now = new Date();
          ...
          images.forEach((img, index) => {
            if (img.url) {
              // Format: Setset_Remix_YYMMDD_HHMM_XX.png
              const paddedIndex = (index + 1).toString().padStart(2, '0');
              const fileName = `Setset_Remix_${dateTime}_${paddedIndex}.png`;
              const fileSize = img.file_size
        ...
        // Check if it's a quota/rate limit error (status 500 with RESOURCE_EXHAUSTED)
        const isQuotaError =
          apiError.status === 500 && apiError.body?.detail?.includes('RESOURCE_EXHAUSTED');

        if (isQuotaError && attempt < maxRetries) {
          // Extract retry delay from error if available, otherwise use exponential backoff
          let retryDelay = Math.pow(2, attempt) * 1000; // 2s, 4s, 8s

          const retryMatch = apiError.body?.detail?.match(/retryDelay.*?(\d+)s/);
          if (retryMatch) {
            retryDelay = parseInt(retryMatch[1]) * 1000;
          }

          console.log(
            `\n⏳ Rate limit hit. Retrying in ${retryDelay / 1000}s... (Attempt ${attempt}/${maxRetries})`
          );
          await new Promise((resolve) => setTimeout(resolve, retryDelay));
          continue;
        }

        // If not a quota error or max retries reached, handle the error
        console.error('\n❌ === NANO BANANA PRO REMIX FAILED ===');
        console.error('API Error:', apiError.message || apiError);

        if (apiError.status === 422) {
          console.error('Validation Error Details:');
          if (apiError.body?.detail) {
            console.error(JSON.stringify(apiError.body.detail, null, 2));

            // Check for content policy violation
            const detail = apiError.body.detail[0];
            if (detail?.type === 'content_policy_violation') {
              console.error('⚠️  Content policy violation detected');
              console.error(
                'Note: The API has flagged this prompt for content policy reasons.'
              );

              return res.status(422).json({
                error:
                  'Content policy violation: The prompt was flagged by the AI service. Try rephrasing your request.',
                details:
                  'API flagged the prompt. Try using more general terms or different phrasing.',
              });
            }
          }

          // Check if it's an image format issue
          if (image_urls.some((url) => url.startsWith('data:'))) {
            console.error('Note: Base64 images detected. Ensure they are properly formatted.');
          }

          return res.status(422).json({
            error: 'Invalid request parameters. Check image format and prompt.',
            details: apiError.body?.detail,
          });
        }
    ...
    console.error('Error in Remix API route:', error);

    // Check if it's a quota exhaustion error
    if (error.status === 500 && error.body?.detail?.includes('RESOURCE_EXHAUSTED')) {
